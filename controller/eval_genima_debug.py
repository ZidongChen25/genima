...
class GenimaEvalWorkspace(Workspace):
    def __init__(
        ...
        # Create a separate config for the environment to enforce action_sequence=1
        # This allows us to step through the environment one action at a time
        # and check for success/termination early.
        env_cfg = copy.deepcopy(self.train_cfg)
        env_cfg.action_sequence = 1
        
        self.eval_env = self.env_factory.make_eval_env(
            env_cfg, self.eval_cfg.controller_ckpt
        )
                ...
                while not termination:
                    ...
                    with torch.inference_mode(), robobase_utils.eval_mode(
                        ...
                        for action in actions:
                            # Unsqueeze action to match expected shape (1, 8)
                            ...
                            # Save step images
                            if self.eval_cfg.save_obs_image:
                                cameras = self.train_cfg.env.cameras
                                for cam in cameras:
                                    # obs[f"{cam}_rgb"] shape: (T, C, H, W)
                                    # We want the most recent frame (last in stack)
                                    if f"{cam}_rgb" in obs:
                                        # Create camera-specific directory
                                        cam_dir = os.path.join(self.eval_cfg.save_image_path, cam)
                                        os.makedirs(cam_dir, exist_ok=True)
                                        
                                        img_array = obs[f"{cam}_rgb"][-1]
                                        img_array = np.transpose(img_array, (1, 2, 0))
                                        img = Image.fromarray(img_array)
                                        img.save(
                                            os.path.join(
                                                cam_dir,
                                                f"step_ep{global_episode}_step{episode_step}.png",
                                            )
                                        )
                            ...
                            if termination or info.get("task_success", 0) or reward > 0.9:
                                termination = True
                                break
